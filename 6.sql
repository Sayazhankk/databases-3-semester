...
-- 4 rows, because Tom has null value on dept_id
--3.2
...
--because query shows everyoe including employees 
...
--lab question
--inner returns the same rows for both, while left returns left and the same for both
--Generating schedules
--these two are like filters:on decides which rows are considered a match and where filters
--15
--for columns with the same name
--if the new column would be added it may use it and change the query result
SELECT * FROM A RIGHT JOIN B 
ON A.id = B.id;
--it will show everything even with null values